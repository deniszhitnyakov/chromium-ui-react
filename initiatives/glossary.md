...
- **ticket** — a single packed problem with a per-initiative number (`#0001`, `#0002`, …) and a long-form description at `initiatives/<initiative>/tickets/NNNN-<short-name>.md`. The ticket file is the source of truth for what the work is about; the kanban card in the same initiative is a thin pointer at it. **Numbering is per-initiative** — each new initiative starts fresh from `#0001`. To cite a ticket from outside its initiative, prefix with the initiative name: `2026-04-28-design-and-styleguide#0017`. Lifecycle: `open → in-progress → done` (or `wontfix`).
