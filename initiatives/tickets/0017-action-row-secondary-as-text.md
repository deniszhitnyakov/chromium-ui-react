...
Working hypothesis (subject to revision when the work starts): direction 3, with an explicit scope statement. The principles.md example is destructive, so the `text` Cancel applies there; the anti-patterns.md #2 example is `action` Save, so `outlined` Cancel stays; readers get a clear rule without a blanket departure from Chromium. If the work turn finds direction 3 too fiddly, direction 2 is the simpler fallback — at the cost of one more documented divergence from Chromium.

## Acceptance hints

- The "Remove bookmark?" example in `docs/docs/styleguide/principles.md` no longer renders Cancel as a pill outlined button next to a destructive pill. It is either `variant="text"` (direction 2 / 3) or rephrased so the visual problem disappears.
...
- Anti-pattern #2 in `docs/docs/styleguide/anti-patterns.md` is reviewed against the new rule and updated if needed.
- All other styleguide / pattern / sample / component live previews that show an action-row pair are reviewed and brought in line. After the sweep, no live preview pairs an `outlined` Cancel with a `destructive` primary (under direction 2 / 3).
...
- Related: ticket #0003 (drop Button `fullWidth` — sweeps the same set of action-row examples; co-implementation worth considering); ticket #0009 (drop Button `tonal`); ticket #0004 (button label length — this ticket also touches Button copy).
