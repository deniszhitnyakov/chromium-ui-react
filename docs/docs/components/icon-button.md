...
- **Don't glue it to a page / panel title in the header.** Parking a settings gear, a "+", or any other `IconButton` immediately next to a `<Header>` / `<PanelHeader>` title is the most common way surfaces stop reading as Chromium-native. Demote to a drill-in `PanelRow` inside the content. See [Styleguide Anti-pattern #16](/styleguide/anti-patterns#16-iconbutton-glued-to-a-title-in-the-header) for the rule and the one narrow case (single `⋮` overflow at the far right, with content between it and the title) where a toolbar `IconButton` is acceptable.
